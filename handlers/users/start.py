...
async def bot_start(message: types.Message):
    ...
    code = db.get_user_uuid(sender_id)
    if not code:
        code = uuid.uuid4().hex[:14]
        db.insert_uuid_for_user(sender_id, code)
...
async def handle_send_once(callback: types.CallbackQuery):
    ...
    db.add_reply_session(sender_id, receiver_id)  # RAM emas, DB ga yozamiz
    await callback.message.edit_text("Habaringizni shu yerda qoldiring.")
...
async def handle_reply_to(callback: types.CallbackQuery):
    receiver_id = int(callback.data.split(":")[1])
    sender_id = callback.from_user.id
    db.add_reply_session(sender_id, receiver_id)
    await callback.message.answer("Habaringizni shu yerda qoldiring.")
    await callback.answer()
...
async def handle_reply(message: types.Message):
    sender_id = message.from_user.id
    receiver_id = db.get_reply_session(sender_id)
    if receiver_id:
        keyboard = InlineKeyboardMarkup().add(
